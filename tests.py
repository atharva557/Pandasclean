...
from pandasclean import find_outliers, find_outliers_zscore, reduce_memory, handle_nan, auto_clean
...
def test_outliers_invalid_strategy_raises():
    df = make_df()
    try:
        find_outliers(df, strategy='invalid')
        assert False, "Should have raised ValueError"
    except ValueError:
        pass
run_test("invalid strategy raises ValueError", test_outliers_invalid_strategy_raises)


# ── find_outliers_zscore tests ─────────────────────────────────────────────────

print("\n── find_outliers_zscore ────────────────────────────────────────────────")

def test_zscore_report_returns_df_and_dict():
    df = make_df()
    result, info = find_outliers_zscore(df, strategy='report')
    assert isinstance(result, pd.DataFrame)
    assert isinstance(info, dict)
run_test("report strategy returns (DataFrame, dict)", test_zscore_report_returns_df_and_dict)

def test_zscore_report_does_not_modify_df():
    df = make_df()
    result, _ = find_outliers_zscore(df, strategy='report')
    assert len(result) == len(df)
run_test("report strategy does not modify DataFrame", test_zscore_report_does_not_modify_df)

def test_zscore_report_has_stats():
    df = make_df()
    _, info = find_outliers_zscore(df, strategy='report')
    for col in df.select_dtypes(include=['number']).columns:
        if isinstance(info[col], dict):
            assert 'mean' in info[col]
            assert 'std' in info[col]
            assert 'lower_bound' in info[col]
            assert 'upper_bound' in info[col]
            assert 'outlier_count' in info[col]
run_test("report contains mean, std, bounds, and outlier_count", test_zscore_report_has_stats)

def test_zscore_drop_removes_outlier_rows():
    df = make_df()
    result, _ = find_outliers_zscore(df, threshold=2.0, strategy='drop')
    assert len(result) < len(df)
run_test("drop strategy removes outlier rows", test_zscore_drop_removes_outlier_rows)

def test_zscore_drop_does_not_mutate_original():
    df = make_df()
    original_len = len(df)
    find_outliers_zscore(df, strategy='drop')
    assert len(df) == original_len
run_test("drop strategy does not mutate original DataFrame", test_zscore_drop_does_not_mutate_original)

def test_zscore_cap_keeps_same_row_count():
    df = make_df()
    result, _ = find_outliers_zscore(df, strategy='cap')
    assert len(result) == len(df)
run_test("cap strategy keeps same number of rows", test_zscore_cap_keeps_same_row_count)

def test_zscore_cap_clamps_values():
    df = make_df()
    result, info = find_outliers_zscore(df, columns=['age'], threshold=2.0, strategy='cap')
    lower = info['age']['lower_bound']
    upper = info['age']['upper_bound']
    assert result['age'].max() <= upper
    assert result['age'].min() >= lower
run_test("cap strategy clamps values within bounds", test_zscore_cap_clamps_values)

def test_zscore_negative_threshold_converted():
    df = make_df()
    result, _ = find_outliers_zscore(df, threshold=-3.0, strategy='report')
    assert isinstance(result, pd.DataFrame)
run_test("negative threshold is converted to positive", test_zscore_negative_threshold_converted)

def test_zscore_zero_std_handled():
    df = pd.DataFrame({'const': [5, 5, 5, 5, 5]})
    _, info = find_outliers_zscore(df, strategy='report')
    assert isinstance(info['const'], str)
run_test("zero std column is handled gracefully", test_zscore_zero_std_handled)

def test_zscore_non_numeric_column_skipped():
    df = make_df()
    _, info = find_outliers_zscore(df, columns=['name'], strategy='report')
    assert isinstance(info['name'], str)
run_test("non-numeric column is skipped with message", test_zscore_non_numeric_column_skipped)

def test_zscore_invalid_strategy_raises():
    df = make_df()
    try:
        find_outliers_zscore(df, strategy='invalid')
        assert False, "Should have raised ValueError"
    except ValueError:
        pass
run_test("invalid strategy raises ValueError", test_zscore_invalid_strategy_raises)

def test_zscore_invalid_input_raises():
    try:
        find_outliers_zscore([1, 2, 3])
        assert False, "Should have raised TypeError"
    except TypeError:
        pass
run_test("non-DataFrame input raises TypeError", test_zscore_invalid_input_raises)
...
print(f"{'─' * 60}\n")
df_test = pd.DataFrame({'col': pd.array([1, 2, None, 4, 5], dtype='Int64')})
result, report = reduce_memory(df_test)
print(report)